...
public class DogRepository {
    public static List<Animal> getAllDogs() throws SQLException {
    ...
    public static Animal insertDog(Animal animal) throws SQLException {
        Connection conn = null;
        PreparedStatement comando = null;
        int righeInserite = 0;
        try {
            conn = DriverManager.getConnection(DBConfig.URL, DBConfig.USER, DBConfig.PPW);
            String query = "INSERT INTO DOG (NAME, AGE, BREED, GENDER, ADOPTION_DATE, BOX, CHECK_IN, IS_ADOPTED, ID_ADOPTER) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            comando = conn.prepareStatement(query);
    ...
    public  static Animal updateDog(Animal animal, int id) throws SQLException {
        Connection conn = null;
        PreparedStatement comando = null;
        int righeAggiornate = 0;
        try {
            conn = DriverManager.getConnection(DBConfig.URL, DBConfig.USER, DBConfig.PPW);
            String query = "UPDATE DOG SET NAME = ?, AGE = ?, BREED = ?, GENDER = ?, ADOPTION_DATE = ?, BOX = ?, CHECK_IN = ?, IS_ADOPTED = ?, ID_ADOPTER = ? " +
                    "WHERE ID = ?";
            comando = conn.prepareStatement(query);
            comando.setString(1, animal.getName());
            comando.setInt(2, animal.getAge());
    ...
    public static boolean deleteDog(int id) throws SQLException {
        Connection conn = null;
        PreparedStatement comando = null;
